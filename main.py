...
from showme import Ui_MainWindow
...
class Toonkor(QMainWindow, Ui_MainWindow):
	def __init__(self):
		...
		self.domain = "https://mangashow.me"
		...
		for i in range(self.toon_view.count()-1, -1, -1):
			if self.toon_view.item(i).checkState() == Qt.Checked:
				crawl_list.append(
					...
					self.toonview_urllist[self.toon_view.item(i).text()]])
		if len(crawl_list) == 0:
			err = QMessageBox()
	...
	def toon_get_source(self, driver, url, per):
		...
		imglist = [soup.find("div",{"class":"subject"}).h1.get_text().strip()]
		title_filter = '\\/<>:?!*"|'
		...
		for tag in taglist:
			if tag.attrs['src'].startswith("http"):
				...
				imglist.append(self.domain + tag.attrs['src'])
		return imglist
	...
	def toon_listing(self, url,th):
		...
		try:
			toonview_list = bs(html, "html.parser").findAll("div",{"class":"slot"})
		except:
			th.error = 2
			return
